...
from core.ws_metrics import increment_ws_broadcast
from core.ws_throttle import throttle_coalesced

logger = logging.getLogger(__name__)

_WS_BYPASS_CELERY = os.environ.get("WS_BYPASS_CELERY", "false").lower() in ("true", "1", "yes")
...
def broadcast_kitchen_order_status_changed(branch_id, message: dict) -> None:
    """``order_status_changed`` olayını asenkron (Celery) veya fallback olarak senkron yollar.
    
    Yüksek frekanslı çağrıları throttle_coalesced ile birleştirir (RAPOR-3 D-7).
    """
    if not branch_id:
        return
    if _WS_BYPASS_CELERY:
        throttle_coalesced(
            ...
            str(branch_id),
            run=lambda: _broadcast_kitchen_order_status_changed_now(branch_id, message),
        )
        return
    try:
        from .tasks import broadcast_kitchen_order_status_changed_task
        throttle_coalesced(
            "order_status_changed",
            str(branch_id),
            run=lambda: broadcast_kitchen_order_status_changed_task.delay(str(branch_id), message),
        )
    except Exception:
        logger.warning("Celery WebSocket status change task dispatch failed, falling back to sync broadcast", exc_info=True)
...
def _broadcast_kitchen_order_status_changed_now(branch_id, message: dict) -> None:
    """Senkron olarak ``order_status_changed`` yayını yapar (Celery task'ından çağrılır)."""
    channel_layer = get_channel_layer()
    if channel_layer is None or not branch_id:
        return
