...
> **Son INGEST:** 2026-08-22 — **Mobil mimari inceleme (cerrahi, kanıtlı):** Waiter JWT refresh + rotation, stale `syncing` kuyruk kurtarma, SQLite yazma hatalarının yukarı fırlatılması. Smart Table `selectTable` `get()` typo, idle timeout sepet temizliği. Stock Man sayım `update_items` payload-hash idempotency, düzenlenebilir PO fiyatında `financial.view_amount`, `transfer.status_changed` artık depo WS grubuna da gider. Güncellenen: [[Index]], [[Mobile_Apps_Family]], [[Mobile_Waiter_App]], [[Smart_Table]], [[Stock_Man_App]], [[Warehouse]], [[WebSocket_Architecture]].
