#!/usr/bin/env python3
"""
Tüm text-slate-* sınıflarını kaldırır.
...
Kullanım:
  python3 scripts/remove-text-slate.py [--dry-run]
"""
...
# text-slate-* ve dark:text-slate-* pattern'leri
PATTERNS = [
    # dark:text-slate-* (önce dark versiyonu kaldır)
    re.compile(r'\bdark:text-slate-\d+\b'),
    # text-slate-*
    re.compile(r'\btext-slate-\d+\b'),
]


def clean_class_string(s: str) -> str:
    """Class string'inden boşluk temizliği yap."""
...
def process_file(filepath: Path, dry_run: bool) -> list[str]:
    ...
    # className string'lerindeki çift boşlukları temizle
    ...
    # Template literal className'leri de temizle
    # className={`foo  bar`} → className={`foo bar`}
...
def main():
    ...
    for filepath in sorted(files):
        ...
        if "text-slate-" not in content and "dark:text-slate-" not in content:
            continue
    ...
    print(f"Değişen dosya: {total_changed}  |  Kaldırılan text-slate: ~{total_removed}")

    if dry_run:
        print(f"\n⚠ Dry run — çalıştır: python3 scripts/remove-text-slate.py")
