  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 text-foreground">{t('menu.title')}</h2>
          <p className="text-sm text-muted-foreground mt-1">{t('menu.description')}</p>
        </div>
        {canManageProducts && (
          <Link href="/menu-management" className="flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 shadow-sm transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl border border-border shadow-sm bg-card border-border">
          <div className="px-4 py-3 border-b border-border border-border">
            <h3 className="text-sm font-bold text-slate-800 text-foreground">{t('menu.categoriesCount', { count: categories.length })}</h3>
          </div>
            ...
            {categories.map(cat => (
              <div key={cat.id} className="px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-slate-800 text-foreground">{cat.name}</span>
                </div>
        ...
        <div className="rounded-2xl border border-border shadow-sm bg-card border-border">
          <div className="px-4 py-3 border-b border-border border-border">
            <h3 className="text-sm font-bold text-slate-800 text-foreground">{t('menu.productsCount', { count: products.length })}</h3>
          </div>
          <div className="divide-y divide-slate-100 max-h-[400px] overflow-y-auto dark:divide-slate-800">
            {products.map(p => (
              <div key={p.id} className="px-4 py-3 flex items-center justify-between">
                <div>
                  <span className="font-medium text-slate-800 text-foreground">{p.name}</span>
