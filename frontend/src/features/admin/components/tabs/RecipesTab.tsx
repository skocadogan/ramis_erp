  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 text-foreground">{t('recipes.title')}</h2>
          <p className="text-sm text-muted-foreground mt-1">{t('recipes.description')}</p>
        </div>
        {canManageRecipes && (
          <Link href="/recipes" className="flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 shadow-sm transition-colors">
      ...
      <VirtualTable
        ...
        header={
          <thead className={virtualTableStickyHeadClass}>
            <tr>
              <th className="text-left px-4 py-3 font-semibold text-slate-600 dark:text-muted-foreground">{t('recipes.table.recipe')}</th>
              <th className="text-left px-4 py-3 font-semibold text-slate-600 dark:text-muted-foreground">{t('recipes.table.product')}</th>
              <th className="text-center px-4 py-3 font-semibold text-slate-600 dark:text-muted-foreground">{t('recipes.table.ingredientCount')}</th>
              <th className="text-right px-4 py-3 font-semibold text-slate-600 dark:text-muted-foreground">{t('recipes.table.cost')}</th>
            </tr>
          </thead>
        }
        emptyState={
        ...
        renderRow={(r) => (
          <>
            <td className="px-4 py-3 font-medium text-slate-800 text-foreground">
      ...
      <Dialog open={!!selectedRecipe} onOpenChange={(v) => !v && setSelectedRecipe(null)}>
        <DialogContent className="max-w-2xl bg-card">
          ...
          {selectedRecipe && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-lg border border-slate-100 bg-muted border-border">
                <div>
                  <span className="text-sm text-muted-foreground block">{t('recipes.menuProduct')}</span>
                  <span className="font-semibold text-slate-800 text-foreground">{selectedRecipe.product_name || t('recipes.independent')}</span>
                </div>
                <div>
              ...
              <div>
                <h4 className="text-sm font-semibold text-slate-800 mb-2 text-foreground">{t('recipes.ingredients')} ({selectedRecipe.ingredients?.length || 0})</h4>
                <div className="rounded-lg border border-border overflow-hidden max-h-60 overflow-y-auto bg-card border-border">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 border-b border-border bg-muted border-border">
                      <tr>
                        <th className="text-left px-4 py-2 font-semibold text-slate-600 dark:text-muted-foreground">{t('recipes.table.ingredient')}</th>
                        <th className="text-right px-4 py-2 font-semibold text-slate-600 dark:text-muted-foreground">{t('recipes.table.quantity')}</th>
                        <th className="text-right px-4 py-2 font-semibold text-slate-600 dark:text-muted-foreground">{t('recipes.table.cost')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedRecipe.ingredients?.map(ing => (
                        <tr key={ing.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50 border-border dark:hover:bg-slate-800/50">
                          <td className="px-4 py-2 text-slate-800 text-foreground">{ing.stock_item_name}</td>
                          <td className="px-4 py-2 text-right text-slate-600 dark:text-muted-foreground">
                            {formatNumber(ing.quantity, 2)} {ing.unit}
                          </td>
                          <td className="px-4 py-2 text-right font-medium text-slate-800 text-foreground">
