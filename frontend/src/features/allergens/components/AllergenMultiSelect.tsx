  ...
  return (
    <div className={className}>
      <div className="flex items-center justify-between gap-2 mb-1">
        <label className="text-sm font-ui-medium text-foreground">{t("label")}</label>
        <button
          type="button"
      ...
      {isLoading ? (
        ...
        <div className="flex flex-wrap gap-1.5 flex-1 overflow-y-auto rounded-md border border-border p-2 bg-slate-50/50 dark:bg-slate-800/30">
          {[...allergens].sort((a, b) => a.name.localeCompare(b.name)).map((a) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => toggle(a.id)}
                className={`rounded-md px-2 py-1 text-xs font-ui-medium border transition-colors ${
                  selected
                    ? "bg-amber-100 border-amber-300 text-amber-900 dark:bg-amber-900/40 dark:border-amber-700 dark:text-amber-100"
                    : "bg-white border-border text-muted-foreground hover:border-amber-200 dark:bg-slate-900 dark:border-slate-700"
                }`}
              >
                {a.name}
