    ...
    <Dialog open={!!recipe} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[90vw] md:max-w-[1200px] lg:max-w-[1400px] w-full h-[90vh] bg-background border-border text-foreground overflow-hidden flex flex-col p-0 shadow-lg">
        <DialogHeader className="p-10 pb-6 shrink-0 border-b border-border/50 bg-muted/10">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1">
              <DialogTitle className="text-xl font-ui-bold text-foreground uppercase tracking-tight flex items-center gap-2">
                <ChefHat className="text-blue-500 shrink-0" size={24} />
                {recipe.name}
              </DialogTitle>
              <p className="text-sm text-muted-foreground font-ui-medium uppercase tracking-widest">
          ...
          <div className="flex flex-wrap items-center gap-4 py-4">
            {recipe.prep_time_minutes > 0 && (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted border border-border text-xs text-muted-foreground">
                ...
                <span className="font-ui-bold">{t('recipeDetail.prepTime')}</span> {recipe.prep_time_minutes} {t('status.late_min_abbr') || 'dk'}
              </div>
            )}
            {recipe.cook_time_minutes > 0 && (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted border border-border text-xs text-muted-foreground">
                <Clock size={14} className="text-orange-400" />
                <span className="font-ui-bold">{t('recipeDetail.cookTime')}</span> {recipe.cook_time_minutes} {t('status.late_min_abbr') || 'dk'}
              </div>
            )}
            {recipe.servings > 0 && (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted border border-border text-xs text-muted-foreground">
                <Users size={14} className="text-emerald-400" />
                <span className="font-ui-bold">{t('recipeDetail.servings')}</span> {recipe.servings}
              </div>
            )}
          </div>
        </DialogHeader>
        ...
        <div className="flex-1 min-h-0 grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border/50 overflow-hidden">
          ...
          <div className="flex flex-col min-h-0">
            <div className="px-8 py-4 bg-muted/40 border-b border-border/50">
              <h4 className="text-xs font-ui-bold uppercase tracking-[0.2em] text-blue-500">
              ...
              {recipe.ingredients?.length > 0 ? (
                recipe.ingredients.map((ing) => (
                  <div key={ing.id} className="flex items-center justify-between p-4 rounded-xl bg-muted/40 border border-border/30 hover:bg-muted/60 transition-colors group">
                    <span className="text-sm font-ui-bold text-muted-foreground group-hover:text-foreground transition-colors">{ing.stock_item_name}</span>
                    <span className="text-sm font-ui-bold tabular-nums bg-muted px-3 py-1.5 rounded-lg border border-border text-foreground">
          ...
          <div className="flex flex-col min-h-0 bg-slate-900/10">
            <div className="px-8 py-4 bg-muted/40 border-b border-border/50">
              <h4 className="text-xs font-ui-bold uppercase tracking-[0.2em] text-orange-500">
              ...
              <div className="space-y-8">
                {recipe.description && (
                  <div className="space-y-3">
                    <h5 className="text-2xs font-ui-bold uppercase tracking-[0.3em] text-muted-foreground/60">{t('recipeDetail.description')}</h5>
                    <div className="text-lg italic text-foreground leading-relaxed whitespace-pre-wrap font-ui-medium">
                      {recipe.description}
                    </div>
                  </div>
                )}
                ...
                {recipe.instructions && (
                  <div className="space-y-3">
                    <h5 className="text-2xs font-ui-bold uppercase tracking-[0.3em] text-muted-foreground/60">{t('recipeDetail.steps')}</h5>
        ...
        <div className="p-4 border-t border-border bg-background flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-6 py-2 rounded-xl text-xs font-ui-bold uppercase tracking-widest text-muted-foreground hover:text-foreground hover:bg-muted transition-colors border border-border"
          >
            {t('inventory.close')}
          </button>
        </div>
