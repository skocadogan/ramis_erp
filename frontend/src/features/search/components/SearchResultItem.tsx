    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="text-sm font-ui-medium text-slate-800 dark:text-slate-100 truncate">
            {item.title}
          </p>
          {badgeLabel && (
            <span className="inline-flex h-5 shrink-0 items-center rounded-4xl border border-border/60 bg-muted/60 px-2 text-[10px] font-ui-medium text-muted-foreground whitespace-nowrap">
              {badgeLabel}
            </span>
          )}
        </div>
