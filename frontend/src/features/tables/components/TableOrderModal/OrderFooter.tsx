    ...
    return (
        <div className="space-y-4">
            {showTotals && totalOrderDiscount > 0 && (
                <div className="flex flex-col gap-1 border-b border-slate-50 dark:border-slate-800 pb-2 mb-1">
                    <div className="flex items-center justify-between text-sm  font-mono">
                        <span>{t('orderSubtotal')}</span>
                        <span className="font-ui-medium font-mono sm:text-lg">
                            {formatAmount(subtotalBeforeOrderDiscount, canViewAmounts)}
                        </span>
                    </div>
                    <div className="flex items-center justify-between text-sm font-ui-semibold text-amber-600 font-mono">
                        <div className="flex items-center gap-1">
                            <Tag size={12} />
                            <span>{t('discount')}</span>
                        </div>
                        <span className="font-ui-bold sm:text-lg">
            ...
            {showTotals && (
                <div className="flex items-center justify-between gap-3">
                    <span className="text-xl font-ui-semibold text-slate-600 font-mono dark:text-slate-300 ">
                        {orders.length > 1 ? t('grandTotal') : t('total')}
                    </span>
                    <span className="text-xl font-ui-bold text-slate-900 dark:text-slate-100 sm:text-4xl font-mono">
            ...
            {showPayment && !readOnly && (
                <>
                    {allowSplitPayment && onToggleSplit && (
                        <label className="flex cursor-pointer items-center gap-2 text-xs font-ui-medium text-slate-600 dark:text-slate-300">
                            <input
                                type="checkbox"
                    ...
                    {useSplitPayment && splitAmounts && onSplitAmountChange ? (
                        <div className="space-y-2.5 rounded-lg border border-border bg-slate-50/80 p-3.5 dark:border-slate-700 dark:bg-slate-800/40 sm:p-4">
                            {PAYMENT_METHODS.map(({ value }) => (
                                <div key={value} className="flex items-center gap-2">
                                    <span className="w-16 shrink-0 text-sub font-ui-semibold text-muted-foreground">
                                    ...
                                    <input
                                        ...
                                        className="min-h-10 flex-1 rounded-md border border-border px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-900"
                                    />
                                </div>
                            ))}
                        ...
                        <div className="space-y-2">
                            <div className="flex gap-2">
                                {PAYMENT_METHODS.map(({ value, icon: Icon }) => (
                                    <button
                                        ...
                                        className={`flex min-h-11 flex-1 touch-manipulation items-center justify-center gap-2 rounded-lg border px-1 py-2.5 text-xs font-ui-semibold transition-all sm:text-sm
                                    ${paymentMethod === value && !isCreditSelected
                                        ? 'border-blue-600 bg-blue-600 text-white'
                                        : 'border-border text-slate-600 hover:border-blue-300 hover:text-blue-600 dark:border-slate-700 dark:text-slate-300'
                                    }`}
                                    >
                            ...
                            {onCreditClick && (
                                <button
                                    ...
                                    className={`flex w-full min-h-11 touch-manipulation items-center justify-center gap-2 rounded-lg border px-3 py-2.5 text-xs font-ui-semibold transition-all sm:text-sm
                                    ${isCreditSelected
                                        ? 'border-violet-600 bg-violet-600 text-white'
                                        : 'border-border text-slate-600 hover:border-violet-300 hover:text-violet-600 dark:border-slate-700 dark:text-slate-300'
                                    }`}
                                >
                                    {tCredit('pos.button')}
                                </button>
                    ...
                    {onCancelAll && (
                        <button
                            ...
                            className="flex w-full touch-manipulation items-center justify-center gap-2 rounded-xl border border-rose-200 px-3 py-2.5 text-sm font-ui-semibold text-rose-600 transition-all hover:bg-rose-50 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 dark:border-rose-800 dark:text-rose-400 dark:hover:bg-rose-950/30"
                        >
                            {isCancelling ? (
                    ...
                    <button
                        ...
                        className={`flex w-full touch-manipulation items-center justify-center gap-2 rounded-xl px-3 py-3.5 text-sm font-ui-bold transition-all sm:text-base
                            ${isPaying || isLoading
