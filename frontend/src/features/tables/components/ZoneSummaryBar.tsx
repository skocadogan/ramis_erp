...
function StatPill({ value, label, colorClass }: { value: number; label: string; colorClass: string }) {
    return (
        <span className={`inline-flex items-center gap-1 text-xs font-ui-medium px-1.5 py-0.5 rounded-full ${colorClass}`}>
            <span className="font-ui-bold tabular-nums">{value}</span>
...
function SkeletonCard() {
    return (
        <div className="rounded-lg border border-border bg-muted/20 px-3 py-2 dark:bg-slate-900 dark:border-slate-700">
            <div className="h-2.5 w-16 bg-slate-200 dark:bg-slate-700 rounded mb-2" />
            <div className="h-5 w-8 bg-slate-200 dark:bg-slate-700 rounded mb-1.5" />
    ...
    return (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
            {allZones.map((z) => {
                ...
                return (
                    <div key={z.id} className={`rounded-lg border px-3 py-2 transition-colors ${isTotal ? 'bg-blue-50 border-blue-200 dark:bg-blue-950/20 dark:border-blue-800/50' : 'bg-white border-border dark:bg-slate-900 dark:border-slate-700'}`}>
                        <div className="flex items-baseline justify-between gap-1 mb-1">
                            <p className={`text-sub font-ui-semibold truncate ${isTotal ? 'text-blue-600 dark:text-blue-400' : 'text-muted-foreground'}`}>{z.name}</p>
                            <p className={`text-base font-ui-bold shrink-0 leading-none ${isTotal ? 'text-blue-700 dark:text-blue-300' : 'text-slate-800 dark:text-slate-100'}`}>{z.total}</p>
                        </div>
                        <div className="flex flex-wrap gap-0.5">
                            ...
                            {z.out_of_service > 0 && <StatPill value={z.out_of_service} label={tStatus('out_of_service_abbr')} colorClass="bg-slate-100 text-muted-foreground dark:bg-slate-800 dark:text-muted-foreground" />}
                        </div>
                        {z.total > 0 && (
