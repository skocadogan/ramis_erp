...
function SkeletonCard() {
    return (
        <div className="rounded-lg border border-border bg-muted/20 px-3 py-2">
            <div className="h-2.5 w-16 bg-accent rounded mb-2" />
            <div className="h-5 w-8 bg-accent rounded mb-1.5" />
                ...
                return (
                    <div key={z.id} className={`rounded-lg border px-3 py-2 transition-colors ${isTotal ? 'bg-blue-50 border-blue-200 dark:bg-blue-950/20 dark:border-blue-800/50' : 'border-border bg-card border-border'}`}>
                        <div className="flex items-baseline justify-between gap-1 mb-1">
                            ...
                            <p className={`text-base font-bold shrink-0 leading-none ${isTotal ? 'text-blue-700 dark:text-blue-300' : ' text-foreground'}`}>{z.total}</p>
                        </div>
                        <div className="flex flex-wrap gap-0.5">
                            ...
                            {z.out_of_service > 0 && <StatPill value={z.out_of_service} label={tStatus('out_of_service_abbr')} colorClass=" text-muted-foreground bg-muted dark:text-muted-foreground" />}
                        </div>
                        {z.total > 0 && (
