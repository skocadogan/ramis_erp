...
const input = "w-full px-3 py-2 bg-slate-50 border border-border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100"
...
export function ChangePasswordModal({ onClose }: ChangePasswordModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[60] p-4" onClick={onClose}>
      <div className="bg-white rounded-lg border border-border w-full max-w-md shadow-lg dark:bg-slate-900 dark:border-slate-700" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 pt-5 pb-2">
          <h2 className="text-base font-ui-semibold text-foreground">{t("title")}</h2>
        ...
        <div className="px-6 pb-6 space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={onClose} className="px-4 py-1.5 text-sm font-ui-medium text-slate-600 hover:bg-slate-100 rounded-md dark:text-slate-300 dark:hover:bg-slate-800">{t("cancel")}</button>
