...
export function ExpiryRiskWidget({
  ...
  const card = compact ? (
    <div
      className={cn(
        ...
        hasRisk
          ...
          : "border-border bg-card dark:border-slate-800",
        className,
      )}
    >
      <div
      ...
      <div className="flex items-center gap-1.5 whitespace-nowrap">
        <span className="text-2xs font-ui-medium text-muted-foreground">{t("expiryWidget.title")}</span>
        <span className="text-xs font-ui-semibold tabular-nums text-foreground">
    ...
    <div
      className={cn(
        "group relative overflow-hidden rounded-xl border border-border/80 p-4 transition-all hover:shadow-md dark:border-slate-800",
        hasRisk ? "bg-amber-50 dark:bg-amber-950/30" : "bg-slate-50 dark:bg-slate-900/40",
        className,
      )}
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="mb-1 text-sub font-ui-medium uppercase tracking-wider text-muted-foreground">
        ...
        <div className={cn("rounded-lg bg-white/60 p-2 dark:bg-slate-900/40", hasRisk ? "text-amber-600" : "text-slate-500")}>
          <Clock size={22} />
        </div>
      </div>
    </div>
