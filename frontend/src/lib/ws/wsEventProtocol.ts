...
/** Parse + dedup + sequence kontrolü; geçerli olay döner, aksi halde null. */
export function acceptWsEvent(
  raw: string | unknown,
  aggregateKey: string,
): NormalizedWsMessage | null {
  const parsed = parseWsMessage(raw);
  if (!parsed) return null;
  if (!dedupByEventId(parsed.eventId)) return null;
  if (!shouldApplySequence(aggregateKey, parsed.sequence)) return null;
  return parsed;
}
